...
if [ "$(uname -m)" == "x86_64" ] && [  "$(uname)" == "Linux" ];then
    ...
    apt update -y
    apt upgrade -y
    apt install -y aria2 python3 busybox zip unzip p7zip-full openjdk-8-jre zipalign zstd bc android-sdk-libsparse-utils xmlstarlet openssl python3-protobuf
    if [ $? -ne 0 ];then
